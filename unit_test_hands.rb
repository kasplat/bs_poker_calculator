...
class TestPair < Test::Unit::TestCase
    ...
    def setup
        ...
        @three_random = Hand.new([Card.new('1', 'Hearts'), Card.new('Ace', 'Hearts'), Card.new('Queen', 'Hearts')])
        @pair_aces_and_fives = @pair_aces.cards.push(Card.new(5, 'Hearts'), Card.new(5, 'Clubs'))
        @triple_aces = @pair_aces.cards.push(Card.new('Ace', 'Spades'))
        puts @pair_aces
        puts @three_random
    end
